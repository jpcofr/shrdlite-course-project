...
module Planner {
    ...
    : PlannerResult[] {
        ...
        if (plans.length) {
            ...
            throw "Planning error: Could not find "    // THROW ONLY THE
                  + "a plan to execute your command."; // FIRST ERROR FOUND
        }
    }

    // Result of the planning towards a DNF formula.
    export interface PlannerResult extends Interpreter.InterpretationResult {
        plan : string[];
    }

    // Stringifies a planner result.
    ...
    class SearchSpace implements Graph<WorldState> {
        outgoingEdges = function (state : WorldState) : SearchEdge[] {
            ...
            if (state.holding == null) { // IF THE ARM IS EMPTY
                for (var sourceCol in state.stacks) {
                    if(state.stacks[sourceCol].length > 0) {
                        ...
                        var originalPosition =
                            ...
                            ? state.stacks[sourceCol][sourceRow - 1]
                            : "floor";
            ...
            else { // IF THE ARM IS HOLDING AN OBJECT
                ...
                for (var destCol in state.stacks) {
                    ...
                    if ( state.stacks[destCol].length == 0                  ||
                        ...
                        // THE HELD OBJECT CAN BE LEGALLY PLACED ON THE STACK
                        var newState : WorldState =
                            { stacks: cloneMatrix(state.stacks) ,
                              holding: null                     ,
                        ...
                        var destinationPosition = destLast != null
                                                ? destLast
                                                : "floor";
